...
-- Get the bounding box (in pixels) of the first non-empty frame of a layer
--  returns {min_x, min_y, max_x, max_y}
local function get_bounds(layer)
    -- Set only this layer to be visible
    hideAllLayers(sprite.layers)
    showLayer(layer,true)
    
    -- get image of first non-empty frame
    local im = Image(sprite)
    for i, frame in ipairs(sprite.frames) do
        im:drawSprite(sprite,i)
        if not im:isEmpty() then break end
    end

    -- If entire layer is empty then bounds are 0,0,0,0
    if im:isEmpty() then
        return {0,0,0,0}
    end
    
    -- Iterate over pixels to find bounding box
    local bounds = nil
    for it in im:pixels() do
        if app.pixelColor.rgbaA(it()) ~= 0 then
            if bounds == nil then
                bounds = {it.x,it.y,it.x,it.y}
            end
            bounds[1] = math.min(bounds[1],it.x)
            bounds[2] = math.min(bounds[2],it.y)
            bounds[3] = math.max(bounds[3],it.x)
            bounds[4] = math.max(bounds[4],it.y)
        end
    end

    -- Adjustment to include the last pixel on right/bottom
    bounds[3] = bounds[3] + 1
    bounds[4] = bounds[4] + 1
    return bounds
end
...
local function main(plugin)
    ...
    -- Get list of layers to export, expanding only top-level groups
    local export_layers = {}
    for i, layer in ipairs(sprite.layers) do
        if layer.isGroup then
            -- Expands only top-level groups
            for i, sublayer in ipairs(layer.layers) do
                table.insert(export_layers,sublayer)
            end
        else
            table.insert(export_layers,layer)
        end
    end
    
    -- Export images (one per tag & layer)
    for i, layer in ipairs(export_layers) do
...
// DO NOT MODIFY - your changes will be overwritten when exporting!
    ...
    for i, layer in ipairs(export_layers) do
        local bounds = get_bounds(layer)
        --bounds = table.concat(bounds,",")
        table.insert(ink_lines,string.format("VAR C_%s = \"%d,%d,%d,%d\"",
            layer.name:gsub(" ","_"),
            bounds[1],
            bounds[2],
            bounds[3],
    ...
    local ink_content = table.concat(ink_lines,"\n")

    local file = io.open(ink_filename,"w")
    if file then
